...
### Chai aur Javascript — Code-Along Notes & Annotated Exercises
...
| Criteria | Status |
|---|---|
| Can explain the prototype chain | ✅ Covered |
| Uses ES6+ syntax fluently (destructuring, spread, optional chaining) | ✅ Covered |
| Understands generators and iterators | ✅ Covered |
| Knows how ES modules differ from CommonJS | ✅ Covered |
...
const user1 = new User("kk", 5);
const user2 = new User("ali", 10);

user1.increment();
user1.printInfo(); // kk has score: 6
...
const user = { username: "kk", email: "kk@gmail.com", score: 99 };
...
const user = { username: "kk", address: null };
...
console.log(apiResponse?.data?.users?.[0]?.name); // "kk"
...
### ES Modules (Modern JavaScript — `.mjs` or `"type":"module"` in package.json)
...
const t = new Teacher("kk", "kk@chai.com", "JavaScript");
console.log(t.getInfo());          // kk — kk@chai.com
console.log(t.addCourse("React")); // kk added: React
...
const fetchUser = (id) => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      if (id > 0) {
        resolve({ id, username: "kk", email: "kk@chai.com" });
      } else {
...
getUser(1);  // { id: 1, username: 'kk', email: 'kk@chai.com' }
getUser(-1); // Error: Invalid user ID
```

---

*Notes prepared while following Chai aur Javascript — Videos 36–51*
*Topics: Prototype Chain · ES6+ Syntax · Iterators & Generators · ES Modules vs CommonJS*
